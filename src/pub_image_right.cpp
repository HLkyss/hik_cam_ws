...
int main(int argc, char** argv){
	ros::init(argc, argv, "pub_image_right");
	ros::NodeHandle nh;
	
	image_transport::ImageTransport it(nh);
//	image_transport::Publisher pub = it.advertise("HikImage_right", 10);
	image_transport::Publisher pub = it.advertise("/camera/right/image_raw", 10);
	...
	unsigned int nIndex = 0; // @TODO 选择相机id
	// 选择设备并创建句柄
...
//	nRet = MV_CC_SetEnumValue(handle, "ExposureAuto", 2);
    nRet = MV_CC_SetFloatValue(handle, "ExposureTime", 9000);
    if(MV_OK != nRet){
    ...
    nRet = MV_CC_SetFloatValue(handle, "Gain", 10.0);
    if(MV_OK != nRet){
